...
public class IntervalT2Engine_Centroid
{
    ...
    static public final short KARNIKMENDEL = 0;
    static public final short ENHANCEDKARNIKMENDEL = 1;
        private final double ENHANCEDKARNIKMENDEL_L0 = 2.4;
        private final double ENHANCEDKARNIKMENDEL_R0 = 1.7;
            
    static public short centroid_algorithm_selector = KARNIKMENDEL;
        ...
        for (int i=0; i<primaryDiscretizationLevel;i++)
        {
            ...
            while(!stopFlag)
            {
                ...
                {if(DEBUG)System.out.println("k = "+k+"  y = "+y+"  x[k] = "+x[k]+"  x[k+1] = " +x[k+1]);
                    if(x[k]<=y && y<=x[k+1])                                //10
                        break;
                    if(x[k]<=y && y<=x[k+1]||(k==0&&Math.abs(x[k]-y)<0.001))                                //10
                        break;
                    //BUG??
                    if(k==primaryDiscretizationLevel-2&&Math.abs(y-x[primaryDiscretizationLevel-1])<0.001)
                            break;
...
//                                     CT2.plotMFs("", new IntervalT2MF_Interface[]{mf}, primaryDiscretizationLevel);
                ...
                {if(DEBUG)System.out.println("SUCCESS! - y = "+y);
                    ...
                    else
                    {
                        y_r = yDash;
                                   ...
                                   // System.out.println("Switch point: "+x[k]);
                }
                else
                {
        ...
        //return new Range(y_l,y_r);	
                    //test
        //System.out.println("CENTROID KM "+y_l+" "+y_r);
    ...
    private Tuple getCentroid_enhanced(IntervalT2MF_Interface mf, double divisor_left, double divisor_right)    //parameter to divide primaryDiscretizationLevel, usually 2.4
    {
        ...
        domainSize = mf.getSupport().getSize();
        //System.out.println(mf.getSupport());
        //setup arrays for primaryDiscretizationLevel
...
//                    if(Math.abs(x[kDash]-y)<0.1)
//                        System.out.println(x[kDash]+" "+y);


                }
                //System.out.println("FOUND "+found);
                kDash=highest_kDash;
                //System.out.println("kDash = "+kDash+"   k = "+k+"   y=" +y+"   x[kDash]= "+x[kDash]);
                if(kDash == k || Math.abs(k-kDash)<0.001)
                {
                    stopFlag = true;
