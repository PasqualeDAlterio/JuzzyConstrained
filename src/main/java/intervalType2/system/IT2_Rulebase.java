...
public class IT2_Rulebase implements FuzzyRulebase
{
...
//    private byte inferenceMethod = 0;  //not used ATM
    ...
    {
        HashMap<Input, HashSet<IntervalT2MF_Interface>> input_to_sets=new HashMap<Input, HashSet<IntervalT2MF_Interface>>();
        HashMap<Output, HashSet<IntervalT2MF_Interface>> output_to_sets=new HashMap<Output, HashSet<IntervalT2MF_Interface>>();
        HashSet<IntervalT2MF_Interface> current_mapping;
        IntervalT2MF_Interface current_set;
        Input current_input;
        Output current_output;
            for(IT2_Rule current_rule : rules)
            {
                for(IT2_Antecedent current_antecendent : current_rule.getAntecedents())
                {
                    ...
                    if(current_mapping==null)
                        current_mapping=new HashSet<IntervalT2MF_Interface>();
                    current_mapping.add(current_set);
                    input_to_sets.put(current_input, current_mapping);
                }
                for(IT2_Consequent current_consequent : current_rule.getConsequents())
                {
                    current_set=current_consequent.getMembershipFunction();
            ...
            for(Input input : input_to_sets.keySet())
                JMathPlotter.plotMFs("Partitioning of "+input.getName(), input_to_sets.get(input).toArray(new IntervalT2MF_Interface[0]), 1000);
            for(Output output : output_to_sets.keySet())
                JMathPlotter.plotMFs("Partitioning of "+output.getName(), output_to_sets.get(output).toArray(new IntervalT2MF_Interface[0]), 1000);
    }
    
    public double getCentroid()
    {
        return this.doReductionCentroid().get(rules.get(0).getConsequents()[0].getOutput()).getAverage();
    }
    ...
    */
    ...
    public TreeMap<Output, Tuple> doReductionCentroid()
    {
        ...
        while(iO.hasNext())
        {
            ...
            Tuple val=iT2EC.getCentroid(overallOutputSet.get(o));
//            if(Double.isNaN(val.getAverage()))
//            System.out.println("IT2 NAN"+ iT2EC.getCentroid(overallOutputSet.get(o)));
            returnValue.put(o, iT2EC.getCentroid(overallOutputSet.get(o)));
        }
        //Plots the non-defuzzified output
//        System.out.println("JUZZY");
IntervalT2MF_Interface output=overallOutputSet.get(outputs.iterator().next());
//System.out.println("OUTPUT SUPPORT "+output.getSupport()+" centroid "+output.getCentroid(100).getAverage()+" Return value "+returnValue.get(returnValue.firstKey()));
//        CT2.plotMFs("Juzzy result", new IntervalT2MF_Interface[]{output}, 1000);
       // System.out.println("Test "+ overallOutputSet.get(outputs.iterator().next()).getLMF().getDefuzzifiedCentroid(100));
        return returnValue;
    }    
