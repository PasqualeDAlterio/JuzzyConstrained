...
import type1.sets.T1MF_InferenceResult;
...
public class T1_Rule implements Serializable
{
    ...
    {
        double fs=getFStrength(MINIMUM);
        Set<Output> key_set=consequents.keySet();
        T1MF_Interface consequent=consequents.get(key_set.iterator().next()).getMF();
        return new T1MF_InferenceResult("", consequent, fs);
    }
    ...
    public double getFStrength(byte tNorm)
    {
        ...
        else    //use minimum
        {
            ...
            {
            	if (antecedents[i].getInput().getInputMF() instanceof T1MF_Singleton) {
            		if(DEBUG)System.out.println("Antecedent "+antecedents[i].getMF().toString()+" gives a FS of: "+antecedents[i].getFS()+" with an input of: "+antecedents[i].getInput().getInput());
