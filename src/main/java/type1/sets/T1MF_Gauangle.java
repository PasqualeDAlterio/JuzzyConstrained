 ...
 */
public class T1MF_Gauangle extends CIT2_Generator_Prototype
{
    ...
    public T1MF_Gauangle(String name, double start, double center, double end)
    {
        ...
        maxPoint=new Tuple[]{new Tuple(center, center)};
    ...
    @Override
    public T1MF_Gauangle clone()
    {
        T1MF_Gauangle result=new T1MF_Gauangle(name, start, center, end);
    ...
    {
        CIT2_Generator shifted_function=new T1MF_Gauangle(name, start+shifting_step, center+shifting_step, end+shifting_step);
        if(isLeftShoulder)
        {
            //If it is a left shoulder, conceptually the support doesn't have an end on the left side, so it must be keept unchanged
            shifted_function.setLeftShoulder(true);
            shifted_function.setSupport(new Tuple(this.support.getLeft(), shifted_function.getSupport().getRight()));
        }
        if(isRightShoulder)
        {
            shifted_function.setRightShoulder(true);
            shifted_function.setSupport(new Tuple(shifted_function.getSupport().getLeft(), this.getSupport().getRight()));
        }
