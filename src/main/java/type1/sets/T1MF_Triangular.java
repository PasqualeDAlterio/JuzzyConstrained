...
public class T1MF_Triangular extends CIT2_Generator_Prototype
{
    ...
    {
        super(n);
        if(peak>end||start>peak||(peak==end&&peak==start))
            throw new IllegalArgumentException("Start must be smaller than peak; peak must be smaller than end "+start+" "+peak+" "+end);
        if(start==peak)
            isLeftShoulder=true;
        if(peak==end)
        ...
        maxPoint=new Tuple[]{new Tuple(peak, peak)};
    }       
    
    
    public T1MF_Triangular clone()
    {
        T1MF_Triangular clone= new T1MF_Triangular(name, start, peak, end);
        clone.isLeftShoulder=isLeftShoulder;
        clone.isRightShoulder=isRightShoulder;
        //clone.setSupport(support.clone());
        return clone;
    }
    ...
    {
        T1MF_Triangular result= new T1MF_Triangular(name, start+shifting_factor, peak+shifting_factor, end+shifting_factor);
        if(isLeftShoulder)
        {
